...
export async function metaGet(path, params = {}) {
  assertMetaConfig();
  const url = new URL(`https://graph.facebook.com/${config.meta.apiVersion}/${String(path).replace(/^\\/+/, '')}`);
  for (const [key, value] of Object.entries({ ...params, access_token: config.meta.accessToken })) {
...
export async function getTokenStatus() {
  ...
  const response = await fetch(url);
  if (!response.ok) throw new Error(`Meta Token 檢查失敗 ${response.status}`);
  ...
  return {
    ...
    expiresAt,
    remainingDays: expiresAt ? Math.max(0, Math.ceil((expiresAt * 1000 - Date.now()) / 86400000)) : null,
  };
}
