...
## 2026-09-23 · 統計區間頁與成效預覽加上客戶／廣告名稱

**背景**：個別回報進到「選擇統計區間」後，整頁看不到是哪個客戶、哪支廣告；成效預覽的標題只有「成效預覽」四個字。截圖回報給客戶前無法確認對象。

**決定**：
- 統計區間頁 `h1` 下加一行 `#periodContext`：統一回報顯示客戶名稱，個別回報顯示「客戶 › 廣告名稱」。
- 成效預覽面板的標題改為廣告名稱，「成效預覽」降為上方小字。
- 名稱只用 `textContent` 或 `esc()` 輸出，不會有 HTML 注入。
...
- 圖片回報：前端用 canvas 繪製 1080px 寬 JPG（月份標題、廣告名稱、日期、指標格子、產出時間），不需要後端或任何套件。
  - 按鈕為「下載圖片」，直接下載 JPG（使用者要求，原本的系統分享表已移除）；也可長按圖片存到相簿。
  - 第一次下載時標記已回報，`metadata.format = 'image'`；再次下載不重複寫入紀錄。
  - 檔名 = 廣告名稱 + 統計區間（例：`私訊 PS65 Ada V2_2026-09-01~2026-09-23.jpg`），移除檔案系統不允許的字元。曾一度改成純英文檔名，因為測試時中文檔名變成「download」；後來查明是測試容器沒設 UTF-8 語系，不是瀏覽器問題，已改回中文。
